...
const timeToAnswer = ref(5)
const maxTimeToAnswer = ref(20)
...
function gameOver(result) {
  clearTimeout(gameTimer.value)
  if (result) router.push({ name: 'gameover', params: { resultgame: 'win' } })
  else router.push({ name: 'gameover', params: { resultgame: 'lose' } })
}
  ...
  <div class="px-4 text-center">
    <div class="col-lg-6 mx-auto">
      <div class="d-grid gap-2 justify-content-sm-center">
        ...
        <RouterLink class="btn btn-secondary btn-lg px-4 gap-3" to="/gameover/lose">Закончить игру
        </RouterLink>
      </div>
    </div>
  </div>
